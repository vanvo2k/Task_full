...
const Handle_0 = async (req, res) => {
  try {
    // ++++++++++++++++++++++++++++++++++++ DreamShip paste sheets =========== OK
    const { data } = await axios.get(
      "https://dreamship.com/page-data/en/catalog/item/7/page-data.json"
    ); // get place
    const DataFinaly = data.result.data.catalogItem.data.item_variants;
    ...
    console.log("Đã update data handle-0 on Google-Sheet !");
  } catch (error) {
    res.status(400).json({
      Message:
...
const Handle_1 = async (req, res) => {
  try {
    ...
    let count_US = 5; // US - PrintWay + Merchize
    for (let i = 3; i <= 10; i++) {
...
const Handle_4 = async (req, res) => {
  try {
    ...
    console.log("Đã update data handle-4 on Google-Sheet !");
  } catch (err) {
    return res.status(400).json({
      message:
        "Loi He Thong =============================================+++> Handle-4",
    });
  }
};
const Handle_5 = async (req, res) => {
  try {
    ...
    console.log("Đã update data handle-5 on Google-Sheet !");
  } catch (err) {
    return res.status(400).json({
      message:
        "Loi He Thong =============================================+++> Handle-5",
    });
  }
};
