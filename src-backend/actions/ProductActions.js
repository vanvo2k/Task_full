...
exports.exportProducts = ({numOfItems, page, limit, query, status, rank, price, availableText, sortBy, type, category = '', brandType, excludeIds = [], aggs = {}, market = 'us'}) => {
    ...
    return ElasticSearch.searchItemsToExport({
        ...
        const {items} = result
        return Promise.resolve(Item.find({ _id: { $in: items } }).select('name thumbnail price description -_id ASIN'))
    }).then(items => {
        const csv = json2csv({ data: items, fieldNames: ['title', 'image', 'ASIN', 'price', 'description'], fields: ['name', 'thumbnail', 'ASIN', 'price', 'description'] })
        return csv
    })
}
