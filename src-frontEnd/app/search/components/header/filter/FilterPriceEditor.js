...
class FilterRankEditor extends Component {
    ...
    render() {
        ...
        return (
            <div className={classNames(["FilterPriceEditor", className])}>
                <Popover className={className} placement="bottom" isOpen={isOpen} target="FilterPrice"
                         toggle={this._handleToggle.bind(this)}>
                    <PopoverHeader><FormattedMessage id="searchItem.header.price_is_between"/></PopoverHeader>
                    <PopoverBody>
                        <Form className="FormFilterPrice" inline onKeyUp={this._handleSubmitForm.bind(this)}>
                            <FormGroup>
...
export default FilterRankEditor;