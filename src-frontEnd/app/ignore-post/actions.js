import {
    ...
    FETCH_LIST_IGNORE_ITEMS_FAILURE,
    FETCH_LIST_IGNORE_ITEMS_REQUEST,
    FETCH_LIST_IGNORE_ITEMS_SUCCESS,
    SHOW_ITEM_DETAIL,
    INIT_SETUP_META,
    CHANGE_FIELD_NAME_SORT_BY
} from "./actionTypes";
import {getPaginationControlData} from "./selectors";
import {_getProductsIgnore} from "../../services/FavoriteServices";
import {_getProductDetail} from "../../services/ProductServices";
import {parseSearchQuery} from "../../helpers/RouterHelper";
import isEmpty from "lodash/isEmpty";

export const fetchProducts = (sortBy = '') => (dispatch, getState) => {
    ...
    dispatch({
        type: FETCH_LIST_IGNORE_ITEMS_REQUEST,
        enableLoadingBar: true
    });

    return _getProductsIgnore(args)
        .then(result => {
            dispatch({
                type: FETCH_LIST_IGNORE_ITEMS_SUCCESS,
                result,
                enableLoadingBar: true
            });
        ...
        .catch(error => {
            dispatch({
                type: FETCH_LIST_IGNORE_ITEMS_FAILURE,
                error,
                enableLoadingBar: true
            });
