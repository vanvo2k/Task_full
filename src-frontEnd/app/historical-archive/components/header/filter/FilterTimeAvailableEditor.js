...
class FilterTimeAvailableEditor extends Component {
    ...
    _isOutsideRangeDate = (date) => {
        ...
        const dateCurrent = currentDate.get('date');
        const dateCurrentMoment = moment.isMoment(dateCurrent) ? dateCurrent : moment(dateCurrent, 'DD-MM-YYYY')
        const oneDayAfterCurrentDate = dateCurrentMoment.add(1, 'day')
        const merchAvailable = moment('01-09-2015', 'DD-MM-YYYY');

        return !date.isBetween(merchAvailable, oneDayAfterCurrentDate) || date.isSame(oneDayAfterCurrentDate, 'day');
    };
    ...
    _isActiveButton({from, to}) {
        ...
        const fromPast = parsePastTimeFromADate(fromT, dateHistory);
        const toPast = parsePastTimeFromADate(toT, dateHistory);
    ...
    render() {
        const {className, isOpen, current, isMobile, historicalDate} = this.props;
        ...
        const initialVisibleMonth = dateHistory ? moment(dateHistory, 'DD-MM-YYYY') : moment()
