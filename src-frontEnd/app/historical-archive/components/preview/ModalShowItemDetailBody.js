...
class ModalShowItemDetailBody extends Component {
    ...
    render() {
        const {item, isMobile, historicalDate} = this.props;
        const {id} = this.props;
            ...
            <ModalBody className={classNames("ModalShowItemDetailBody Body", `c-${category}`, {cropped})}>
                <Row>
                    ...
                    <Col xs={12} sm={8} className="BodyRight">
                        <div className="WrapperBodyRight">
                            ...
                            <span><FormattedMessage
                                id="item.first_available"/>:</span> {timeHuman(item.get("available"), 'DD/MM/YYYY')}
                            </div>

                            <div className="Listed Updated-at">
                            <span><FormattedMessage
                                id="item.updated_at"/>: </span> <TimeAgo time={item.get("updated")}/>
                            </div>
                        </div>
                    </Col>
                </Row>
                ...
                <div className="ranks">
                    <ChartRankItem filtering loading={loading} historyDays={7} height={80}
                                   ranks={item.get('ranks') || List()} historicalDate={historicalDate} />
                </div>
            </ModalBody>
        )
    }
